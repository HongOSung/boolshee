...
<c:set var="pageTitle"><spring:message code="comUssUmt.userManage.MyMembertitle"/></c:set>
<!DOCTYPE html>
<html>
<head>
...
function fnLinkPage(pageNo){
    ...
    document.listForm.action = "<c:url value='/uss/umt/EgovMyMberManage.do'/>";
    document.listForm.submit();
}
...
<form name="listForm" action="<c:url value='/uss/umt/EgovMyMberManage.do'/>" method="post" onSubmit="fn_egov_search_bbssj(); return false;"> 
<div id="board">
		...
		<ul>
			...
			<li><!-- 조건 -->
                <select name="searchCondition" id="searchCondition" title="<spring:message code="comUssUmt.userManageSsearch.searchConditioTitle" />"><!--  -->
                    <option value="0" <c:if test="${userSearchVO.searchCondition == '0'}">selected="selected"</c:if> ><spring:message code="comUssUmt.userManageSsearch.searchMberId" /></option><!-- 회원ID  --> 
                    <option value="1" <c:if test="${userSearchVO.searchCondition == '1'}">selected="selected"</c:if> ><spring:message code="comUssUmt.userManageSsearch.searchMemberNm" /></option><!-- Name -->
                </select>
			</li>
			<!-- 검색키워드 및 조회버튼 -->
			<li>
...
<table class="board_list" summary="<spring:message code="common.summary.list" arguments="${pageTitle}" />">
	...
	<tr>
	    ...
	    <td>
	    	<c:out value="${result.tupyoso}"/>
	    	<c:out value="${result.gepyoso}"/>
	    	<c:out value="${result.pllngplce}"/>
	    	<c:out value="${result.contents}"/>
	    </td><!-- 3 사전투표소 -->
	    <td><c:out value="${result.day}"/></td><!-- 4 사전투표 참관날짜 -->
	    <td><c:out value="${result.ampm}"/></td><!-- 5 오전/오후 -->
		<td>
			<c:out value="${fn:substring(result.telNo,0,3)}"/>&nbsp;&nbsp;<c:out value="${fn:substring(result.telNo,3,7)}"/>&nbsp;&nbsp;<c:out value="${fn:substring(result.telNo,7,11)}"/>
		</td><!-- 6 전화번호 -->
