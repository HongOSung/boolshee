...
function fn_egov_regist_bbs(form){
	...
	if(form.bbsNm.value == null || form.bbsNm.value ==''){
		alert('알림,행사,질문 명을 입력해 주세요!');
		form.bbsNm.focus();return false;
	}
	if(form.action.value == null || form.action.value ==''){
		alert('알림,행사,질문 내용을 입력해 주세요!');
		form.action.focus();return false;
	}
...
function fnInsert(form){
	if(confirm("<spring:message code="common.regist.msg" />")){
/// 이상하게 아래 ajax가 동작하지 않음 2021.11.30 ?????????????????????????????????????
		$.ajax({
    		type:"POST",
    		...
    		success:function(returnData, status){
    			if(status == "success") {
    				if(returnData.usedCnt > 0 ){
    					alert("<spring:message code="fail.common.helpRqstDpKey" />"); // 이미 같은 도움 요청 명이 등록되어 있습니다.
    					return false;
    				}else{
    					form.submit();
    					return true;
...
<body onLoad="fn_egov_init();" document.boardMasterVO.bbsNm.focus()">

<!-- javascript warning tag  -->
<noscript class="noScriptTitle"><spring:message code="common.noScriptTitle.msg" /></noscript>

<form:form commandName="boardMasterVO" action="${pageContext.request.contextPath}/cop/bbs/insertBBSMasterHp.do" method="post" 
onSubmit="fn_egov_regist_bbs(document.forms[0]); return false;"
>  
	...
	<tbody>
		...
		<tr>
			...
			<td class="left">
			    ...
			    <input name="mberId" id="mberId" type="hidden" value="${mberId }" readonly="true" />   
			</td>
		</tr> 
...
<%--	<button name="updatebtn" class="updatebtn" type="button" 
	style="text" action="${pageContext.request.contextPath}/cop/bbs/insertBBSMasterHp.do"> 도움요청 등록
</button>++++++++++++ --%>
	<div class="btn">
