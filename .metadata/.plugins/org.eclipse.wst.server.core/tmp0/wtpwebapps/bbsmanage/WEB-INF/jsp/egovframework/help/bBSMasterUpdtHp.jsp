...
<form:form commandName="boardMasterVO" action="${pageContext.request.contextPath}/cop/bbs/updateBBSMasterHp.do" method="post" onSubmit="fn_egov_updt_bbs(document.forms[0]); return false;">  
<div id="wTableFrm">
	...
	<tbody>
		...
		<!-- 도움요청 내용-->
		<c:set var="BBS_NMtt"><c:out value='${boardMasterVO.BBS_NM}'/></c:set>
		<b><p style="color:red; margin-bottom: 8px; ">${BBS_NMtt}</p></b> 
	...
	<!-- 하단 버튼 --> 
	<%-->>>>>>>>${boardMasterVO.mberId }========${mberId_SS}<<<<<<<<<< --%>
	<c:choose>
	<c:when test="${mberId_SS eq boardMasterVO.mberId }">
		<div class="btn">
			...
			<span class="btn_s"><a href="<c:url value='/cop/bbs/deleteBBSMasterHp.do?bbsId=${boardMasterVO.bbsId}'/>"  title="<spring:message code="button.delete" />  <spring:message code="input.button" />"><spring:message code="button.delete" /></a></span><!-- 삭제 -->
		</div><div style="clear:both;"></div>
	</c:when> 
    <c:otherwise>
		<div class="btn">
			<span class="btn_s"><a href="<c:url value='/uss/umt/EgovMberInsertViewHp.do?bbsId=${boardMasterVO.bbsId}'/>"  title="<spring:message code="button.boolsheeHelper" />  <spring:message code="input.button" />"><spring:message code="button.boolsheeHelper" /></a></span><!-- 응답등록 -->
