...
<c:set var="pageTitle"><spring:message code="title.listWatchGepyoso"/></c:set>
<!DOCTYPE html>
<html>
<head>
...
function fn_egov_select_linkPage(pageNo){
	...
	document.BBSMasterForm.action = "<c:url value='/cop/bbs/selectGepyosoResponseInfs.do'/>";
...
<form name="BBSMasterForm" action="<c:url value='/cop/bbs/selectGepyosoResponseInfs.do'/>" method="post" onSubmit="fn_egov_search_bbssj(); return false;"> 
<div id="board">
		...
		<ul>
			<li>
				<select name="searchCnd" title="<spring:message code="title.searchCondition" /> <spring:message code="input.cSelect" />">
					<option value="0"  <c:if test="${searchVO.searchCnd == '0'}">selected="selected"</c:if> ><spring:message code="comCopBbs.boardMasterVO.list.gepyoso" /></option><!-- 개표소 -->
					<option value="1"  <c:if test="${searchVO.searchCnd == '1'}">selected="selected"</c:if> ><spring:message code="comCopBbs.boardMasterVO.list.gepyosoWatch" /></option><!-- 개표 참관인 -->
				</select>
			</li>
			<!-- 검색키워드 및 조회버튼 -->
			<li>
	...
	<thead>
	<tr>
		<th><spring:message code="table.gepyoRedog" /></th><!-- 참관인-->
	...
	<h1>개표  <font color="blue">참관인</font> <font color="red"><spring:message code="title.list" /></font></h1>
	</tr>
	</thead>
	<tbody class="ov">
	...
	<tr>
		<td class="left"><a href="<c:url value='/cop/bbs/updateGepyosoResponseView.do?no=${resultInfo.no}&mberId=${resultInfo.mberId}'/>" >	
