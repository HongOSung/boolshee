...
<c:set var="pageTitle"><spring:message code="comUssUmt.boolsheeManage.reNttitle"/></c:set>
<!DOCTYPE html>
<html>
<head>
...
function fnInsert(form){
	...
	*/
	...
	if(confirm("<spring:message code="common.regist.msg" />")){
		$.ajax({
    		type:"POST",
    		url:"<c:url value='/uss/umt/EgovBbsMemberDpKeyCnfirmAjaxNt.do' />",
    		data:{  	
    			"mberId": $("#mberId").val(),
    			"bbsNm_S": $("#bbsNm_S").val()
    		},
    		...
    		success:function(returnData, status){
    			if(status == "success") {
    				...
    				if(returnData.usedCnt > 0 ){
    					alert("<spring:message code="fail.common.recommendNtDpKey" />"); // 이 알림,행사,질문에 같은 이름으로 등록되어 있습니다.
    					return false;
    				}else{
    					form.submit();
    					return true;    				
...
<form:form commandName="mberManageVO" action="${pageContext.request.contextPath}/cop/bbs/insertBbsMemberNt.do" name="mberManageVO"  method="post" onSubmit="fnInsert(document.forms[0]); return false;"> 

<div id="wTableFrm">
	<!-- 타이틀 -->
	<%--<h2>${pageTitle} <spring:message code="title.create" /></h2> --%>
	<br><h1><font color="green" size="3em">국회청원</font>, 알림, 행사, 질문 <h1><font color="red" size="2em">응답등록</font></h1><!-- 목록 --><!-- 내용 -->
	...
	%>
		<c:set var="title"><spring:message code="comUssUmt.boolsheeManageRegist.reNtbbsNm"/></c:set>
		...
		<!-- 알림,행사,질문 내용 -->
		<c:set var="title"><spring:message code="comUssUmt.boolsheeManageRegist.reNtaction"/></c:set>
		<c:forEach var="bbsNm_result" items="${bbsNm_result}" varStatus="status" begin="0" end="5" step="1">
		<tr>
		...
		<!-- 알림,행사,질문 응답 내용 -->
		<c:set var="title"><spring:message code="comUssUmt.boolsheeManageRegist.reNtcontents"/></c:set>
		<tr>
