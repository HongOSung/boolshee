...
function fnInsert(form){
	/*
	alert('00 form.gepyoso_S.value='+form.gepyoso_S.value);
   ...
   // }	
	if(confirm("<spring:message code="common.regist.msg" />")){
		$.ajax({
    		type:"POST",
    		url:"<c:url value='/uss/umt/EgovGepyosoReponseDpKeyCnfirmAjax.do' />",
    		data:{
    			"no": $("#no").val(),
    			"mberId": $("#mberId").val()			
    		},
...
<form:form commandName="mberManageVO" action="${pageContext.request.contextPath}/cop/bbs/insertGepyosoResponse.do" name="mberManageVO"  method="post" onSubmit="fnInsert(document.forms[0]); return false;"> 
...
<div id="wTableFrm">
	...
	<tbody>
		...
		<!-- 그룹아이디 -->
		<c:set var="title"><spring:message code="comUssUmt.boolsheeManageRegist.gepyoso"/></c:set>
		<c:forEach var="bbsNm_result" items="${bbsNm_result}" varStatus="status" begin="0" end="15" step="1">
			<tr>				
				<th><label for="gepyoso_dm">${title}</label></th>
				<td class="left">
	                    <%--<c:out value="${bbsNm_result.gepyoso}"/> --%>
	                    <input type="hidden" name="members" value="<c:out value="${bbsNm_result.members}"/>" id="members" /> 
	                    <input type="hidden" name="gepyoso_S" value="<c:out value="${bbsNm_result.gepyoso}"/>" id="gepyoso_S" />			                    				                    	
	                    ...
	                    <input type="text" name="gepyoso" value="<c:out value="${bbsNm_result.gepyoso}"/>" id="gepyoso" />
