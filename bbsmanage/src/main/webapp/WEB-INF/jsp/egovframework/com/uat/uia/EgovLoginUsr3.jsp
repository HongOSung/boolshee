...
function actionLogin() {
	...
	/*
	alert(document.loginForm.id.value+", "+document.loginForm.password.value+", "+document.loginForm.loginIncorrect.value);
	alert(document.loginForm.DB_USER_ID.value+", "+document.loginForm.DB_USER_PW.value+", "
			+document.loginForm.loginIncorrect.value+", "+document.loginForm.fail_common_login.value);	
    	if (document.loginForm.id.value !=document.loginForm.DB_USER_ID.value) {
        	alert("<spring:message code="fail.common.idsearch" />"); <%-- 아이디를 찾을수 없습니다. 회원가입을 하셨는지요? --%>
        }
    	else if (document.loginForm.password.value !=document.loginForm.DB_USER_PW.value) {
        	alert("<spring:message code="fail.common.login.password2" />"); <%-- 비밀번호가 맞지 않습니다. --%>
        }
    	else if (document.loginForm.loginIncorrect.value == "loginIncorrect") {
        	alert("<spring:message code="fail.common.loginIncorrect" />"); <%-- 5회 이상 로그인 접속이 시도 되어 계정이 잠겨습니다! --%>
        }
    	else if (document.loginForm.fail_common_login.value == "fail_common_login") {
...
<div class="login_form">
	...
	<fieldset>
		<img src="<c:url value='/images/egovframework/com/uat/uia/boolshee2.png'/>" style="margin:30px 0 0px 60px" alt="login title image"  title="login title image">
