...
public class EgovLoginController {
	...
	@RequestMapping(value = "/uat/uia/actionLogin_210825.do")
	public String actionLogin(@ModelAttribute("loginVO") LoginVO loginVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		////System.err.println("=============egovLoginConfig.isLock()="+egovLoginConfig.isLock());
		// 1. 로그인인증제한 활성화시 
		if( egovLoginConfig.isLock()){
		    Map<?,?> mapLockUserInfo = (EgovMap)loginService.selectLoginIncorrect(loginVO);
		    //System.err.println("=============mapLockUserInfo="+mapLockUserInfo);
		 // 1. Spring Security 사용자권한 처리
			...
			System.out.println("=======================EgovStringUtil.isNullToString(user.getId())="+EgovStringUtil.isNullToString(user.getId()));
			System.out.println("=======================EgovStringUtil.isNullToString(user.getPassword())="+EgovStringUtil.isNullToString(user.getPassword()));
			String DB_USER_ID=EgovStringUtil.isNullToString(user.getId());
			String DB_USER_PW=EgovStringUtil.isNullToString(user.getPassword());
			System.out.println("=======================EgovStringUtil.isNullToString(user.getPasswordCnsr())="+EgovStringUtil.isNullToString(user.getPasswordCnsr()));
			System.out.println("=======================EgovStringUtil.isNullToString(user..getName())="+EgovStringUtil.isNullToString(user.getName()));

		    //LoginVO user = (LoginVO) EgovUserDetailsHelper.getAuthenticatedUser();
		    ////System.err.println("=============user="+user);
		    /*
		    LOGGER.debug("User Id : {}", user == null ? "" : EgovStringUtil.isNullToString(user.getId()));
			//System.err.println("=======================EgovStringUtil.isNullToString(user.getId())="+EgovStringUtil.isNullToString(user.getId()));
			//System.err.println("=======================EgovStringUtil.isNullToString(user.getPassword())="+EgovStringUtil.isNullToString(user.getPassword()));
			
			String DB_USER_ID=EgovStringUtil.isNullToString(user.getId());
			String DB_USER_PW=EgovStringUtil.isNullToString(user.getPassword());
			request.setAttribute("DB_USER_ID",DB_USER_ID);  
			request.setAttribute("DB_USER_PW",DB_USER_PW); */
			//request.getRequestDispatcher("url").forward(request,response);
			
		    if(mapLockUserInfo != null){	
				String sLoginIncorrectCode = loginService.processLoginIncorrect(loginVO, mapLockUserInfo);
				//System.err.println("=============sLoginIncorrectCode="+sLoginIncorrectCode);
				if(!sLoginIncorrectCode.equals("E")){
					if(sLoginIncorrectCode.equals("L")){
						//System.err.println("=============egovMessageSource.getMessageArgs(\"fail.common.loginIncorrect\", new Object[] {egovLoginConfig.getLockCount(),request.getLocale()})="
					//+egovMessageSource.getMessageArgs("fail.common.loginIncorrect", new Object[] {egovLoginConfig.getLockCount(),request.getLocale()}));
						model.addAttribute("message", egovMessageSource.getMessageArgs("fail.common.loginIncorrect", new Object[] {egovLoginConfig.getLockCount(),request.getLocale()}));
						String loginIncorrect="loginIncorrect";
						request.setAttribute("loginIncorrect",loginIncorrect);
					}else if(sLoginIncorrectCode.equals("C")){
						//System.err.println("=============egovMessageSource.getMessage(\"fail.common.login\",request.getLocale())="
								//+egovMessageSource.getMessage("fail.common.login",request.getLocale()));
						model.addAttribute("message", egovMessageSource.getMessage("fail.common.login",request.getLocale()));
						String fail_common_login="fail_common_login";
						request.setAttribute("fail_common_login",fail_common_login);
						////"fail.common.login"
					}
					//System.err.println("=============egovframework/com/uat/uia/EgovLoginUsr=");
					return "egovframework/com/uat/uia/EgovLoginUsr";
				}
		    }else{
		    	//System.err.println("=============egovMessageSource.getMessage(\"fail.common.login\",request.getLocale())="+egovMessageSource.getMessage("fail.common.login",request.getLocale()));
		    	model.addAttribute("message", egovMessageSource.getMessage("fail.common.login",request.getLocale()));
		    	String fail_common_login="fail_common_login";
				request.setAttribute("fail_common_login",fail_common_login);
		    	return "egovframework/com/uat/uia/EgovLoginUsr";
		...
		if (resultVO != null && resultVO.getId() != null && !resultVO.getId().equals("")) {
			...
			//System.err.println("=============request.getSession().resultVO.getUserSe().concat(resultVO.getId())="+ resultVO.getUserSe().concat(resultVO.getId()));
			//System.err.println("=============request.getSession().toString()="+ request.getSession().toString());
			//System.err.println("=============request.getSession().getId()="+ request.getSession().getId());
			//System.err.println("=============request.getSession().getValueNames()="+ request.getSession().getValueNames());
			//System.err.println("=============request.getSession().getValue(\"password\")="+ request.getSession().getValue("password"));//
			//System.err.println("=============request.getSession().getValue(\"passwordCnsr\")="+ request.getSession().getValue("passwordCnsr"));
	...
	public String actionMain(ModelMap model) throws Exception {
		...
		*/
		...
		if (main_page.startsWith("/")) {
			System.err.println("==========111========forward:/main_page=>/EgovContent.do>=" + main_page);
			return "forward:" + main_page;
		} else {
			System.err.println("==================else,main_page=" + main_page);
			return main_page;
		}
	...
	public String searchPassword(@ModelAttribute("loginVO") LoginVO loginVO, ModelMap model) throws Exception {
		...
		// 1. 비밀번호 찾기
		//boolean result = loginService.searchPassword(loginVO);//메일서버 정상 가동까지는 당분간 보류함 20210830 홍오성
		LoginVO resultVO = loginService.searchPassword1(loginVO);
		////System.err.println("===============resresultVOult="+resultVO.toString()+"<<<<<<<<<<<<<<<=loginVO.getUserSe()>>>>>>>>>>"+loginVO.getUserSe());
		// 2. 결과 리턴
		if (resultVO != null && resultVO.getId() != null && !resultVO.getId().equals("")) {
		//if (result) {
