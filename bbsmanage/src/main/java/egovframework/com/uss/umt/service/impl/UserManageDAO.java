...
public class UserManageDAO extends EgovComAbstractDAO{
     ...
     * @return int 사용가능여부(아이디 사용회수 )
     */
	public int checkIdDplct_BbsMberHp(String mbId_bsNm) {
		return (Integer)selectOne("userManageDAO.checkIdDplct_BbsMberHp", mbId_bsNm);
	}

	public int checkIdDplct_BbsMberNt(String mbId_bsNm) {
		return (Integer)selectOne("userManageDAO.checkIdDplct_BbsMberNt", mbId_bsNm);
	}
	
	public int checkIdDplct_BbsMber(String mbId_bsNm) {
		return (Integer)selectOne("userManageDAO.checkIdDplct_BbsMber", mbId_bsNm);
	}
    
	public int checkIdDplct_noMbid(String no_mbId) {
		return (Integer)selectOne("userManageDAO.checkIdDplct_noMbid", no_mbId);
	}
	
	public int checkIdGepyosoResponse_noMbid(String no_mbId) {
		return (Integer)selectOne("userManageDAO.checkIdGepyosoResponse_noMbid", no_mbId);
	}
	
	public int checkIdSajeonTupyosoResponse_noMbid(String no_mbId) {
		return (Integer)selectOne("userManageDAO.checkIdSajeonTupyosoResponse_noMbid", no_mbId);
	}
	
	public int checkIdDplct_pollWatch(String no_mbId) {
		return (Integer)selectOne("userManageDAO.checkIdDplct_pollWatch", no_mbId);
	}
	
	public int EgovBBSMasterDpKeyCnfirmAjax(String no_mbId) {
		return (Integer)selectOne("userManageDAO.EgovBBSMasterDpKeyCnfirmAjax", no_mbId);
	}
	
	public int checkIdDplct_pollWatchHp(String bbsNm) {
		return (Integer)selectOne("userManageDAO.checkIdDplct_pollWatchHp", bbsNm);
	}
	public int checkIdDplct_pollWatchNt(String bbsNm) {
		return (Integer)selectOne("userManageDAO.checkIdDplct_pollWatchNt", bbsNm);
	}


	/**
     * 입력한 사용자패스워드의 정확여부를 체크하여 사용가능여부를 확인
     * @param checkId 확인대상 패스워드
     * @return int 존재여부(패스워드 사용회수 )
     */
    public int checkPwdDplct(String checkPwd){
        return (Integer)selectOne("userManageDAO.checkPwdDplct_W", checkPwd);
    }
	...
	/**
     * 입력한 추천자아이디의 존재여부를 체크하여 사용가능여부를 확인
     * @param checkId 존재체크대상 아이디
     * @return int 사용가능여부(아이디 사용회수 )
     */
    public int checkRecommenderIdCnfirm(String checkId){
        return (Integer)selectOne("userManageDAO.checkRecommenderIdCnfirm_S", checkId);
    }
