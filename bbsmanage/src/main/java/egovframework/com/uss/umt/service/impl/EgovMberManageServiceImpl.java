...
public class EgovMberManageServiceImpl extends EgovAbstractServiceImpl implements EgovMberManageService {  
	...
	public String insertCOMTNGNRLMBER(MberManageVO mberManageVO) throws Exception  {
		String result = mberManageDAO.insertCOMTNGNRLMBER(mberManageVO);
		return result;
	}
	...
	public String insertGepyosoResponse(MberManageVO mberManageVO) throws Exception  {
		String result = mberManageDAO.insertGepyosoResponse(mberManageVO);
		return result;
	}
	
	public String insertMnaResponse(MberManageVO mberManageVO) throws Exception  {
		String result = mberManageDAO.insertMnaResponse(mberManageVO);
		return result;
	}
	
	public String insertSajeonResponse(MberManageVO mberManageVO) throws Exception  {
		String result = mberManageDAO.insertSajeonResponse(mberManageVO);
		return result;
	}
	...
	@Override
	public MberManageVO selectMberHp(String uniqId) {
		MberManageVO mberManageVO = mberManageDAO.selectMberHp(uniqId);
		return mberManageVO;
	}
	...
	@Override
	public List<MberManageVO> selectMyMberList(UserDefaultVO userSearchVO) {
		return mberManageDAO.selectMyMberList(userSearchVO);
	}
    /**
     * 일반회원 총 갯수를 조회한다.
     * @param userSearchVO 검색조건
    ...
    @Override
	public int selectMyMberListTotCnt(UserDefaultVO userSearchVO) {
    	return mberManageDAO.selectMyMberListTotCnt(userSearchVO);
    }
	...
	public void updateMberHp(MberManageVO mberManageVO) throws Exception {
		...
		String pass = EgovFileScrty.encryptPassword(mberManageVO.getPassword(), EgovStringUtil.isNullToString(mberManageVO.getMberId()));//KISA 보안약점 조치 (2018-10-29, 윤창원)
		mberManageVO.setPassword(pass);
		mberManageDAO.updateMberHp(mberManageVO);
	}
	...
	public void deleteMber(String checkedIdForDel)  {
		...
		//System.err.println("=================delId="+delId.toString()+", ");
		
		for (int i=0; i<delId.length ; i++){		
			
			String [] id = delId[i].split(":");
			//System.err.println("=================id[0]="+id[0]+", id[1]="+id[1]);
		...
		mberManageDAO.deleteMber(checkedIdForDel.substring(6));
		/*
	...
	@Override
	public void deleteMberHp(String checkedIdForDel)  {
		/*String [] delId = checkedIdForDel.split(",");
		for (int i=0; i<delId.length ; i++){
			String [] id = delId[i].split(":");
			if (id[0].equals("USR03")){
				...
				//일반회원삭제
				*/
				mberManageDAO.deleteMberHp(checkedIdForDel);
				/*
			}else if(id[0].equals("USR02")){
				//기업회원삭제
				entrprsManageDAO.deleteEntrprsmber(id[1]);
	...
	@Override
	public void updatePasswordHp(MberManageVO mberManageVO) throws Exception {
		mberManageDAO.updatePasswordHp(mberManageVO);
	}
	...
	@Override
	public MberManageVO selectPasswordHp(MberManageVO passVO) {
		MberManageVO mberManageVO = mberManageDAO.selectPasswordHp(passVO);
		return mberManageVO;
	}
	...
	@Override
	public LoginVO actionLogin(Object loginVO) throws Exception {
	...
	public void deleteGnrMber(UserDefaultVO userSearchVO) throws Exception {
		mberManageDAO.deleteGnrMber(userSearchVO);
	}
